...
Motivation: 为大学教育应用创建专门的评估基准，使用开放许可的教科书内容来测试和提升大型语言模型在教育领域的表现。

Method: 使用43本开放大学教科书构建多语言数据集，通过量化低秩适配器(QLoRa)对约70亿参数的大型语言模型进行微调，并在AI2推理挑战数据集上进行零样本评估。

Result: 开发了OpenStaxQA评估基准，并验证了该数据集可能提升模型在其他任务上的性能。
...
<details>
  <summary>Details</summary>
Motivation: 工业问答系统需要比通用对话模型更高的安全性和可靠性，因为高风险场景中的错误可能导致严重后果。虽然多智能体大语言模型增强了推理深度，但存在迭代不可控和输出不可验证的问题，传统蒸馏方法难以将协作推理能力转移到轻量级可部署的学生模型中。

Method: 将蒸馏建模为马尔可夫决策过程，引入知识图谱作为可验证的结构化先验来丰富状态表示并确保收敛。通过整合协作推理与知识基础，生成高置信度的指令调优数据，并将推理深度和可验证性联合蒸馏到适合边缘部署的紧凑学生模型中。

Result: 在工业问答数据集上的实验表明，KG-MASD相比基线方法将准确率提高了2.4%到20.1%，并显著增强了可靠性，能够在安全关键的工业场景中实现可信赖的AI部署。

Conclusion: KG-MASD方法通过知识图谱引导的多智能体系统蒸馏，成功解决了工业问答系统中协作推理能力蒸馏的挑战，为安全关键工业场景提供了可靠且可部署的AI解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 开放式调查回复在营销研究中很有价值，但低质量回复不仅增加人工筛选负担，还可能导致错误结论。现有自动评估方法主要针对LLM生成文本，不适合评估具有独特特征的人类回复。

Method: 两阶段评估框架：第一阶段过滤无意义回复，第二阶段使用LLM能力评估三个维度（努力程度、相关性和完整性），基于真实调查数据的实证分析。

Result: 在英语和韩语数据集上验证，该框架不仅优于现有指标，还显示出与专家评估的高度相关性，在实际应用中具有高实用性。

Conclusion: 该框架专门针对人类调查回复设计，能有效评估回复质量，支持实际应用如质量预测和回复拒绝，与专家评估高度一致。
...
Conclusion: CoT Referring策略有效增强了多模态大语言模型在复杂指代任务中的跨模态推理能力，为MLLM能力评估提供了新的基准。
...
<details>
  <summary>Details</summary>
Motivation: 在特定领域中，同一词语在不同上下文和领域中可能有不同含义和表示。虽然生成式AI和Transformer架构能生成上下文嵌入，但从头预训练这些模型计算成本高昂。因此需要为科学领域找到更高效的词表示和分词方法。

Method: 构建了一个包含多个下游任务和相关数据集的评估套件，使用该套件测试了各种词表示和分词算法在科学领域的表现。

Result: 研究开发了一个全面的评估框架，能够系统评估不同词表示和分词方法在科学领域NLP任务中的效果。
...
<details>
  <summary>Details</summary>
Motivation: 解决印度低资源语言缺乏翻译资源的问题，探索如何改善低资源语言到高资源语言的翻译

Method: 结合CKA（中心核对齐）和REPINA正则化方法，在Aya-23 8B模型上使用QLoRA进行零样本、少样本和微调实验

Result: 使用TRepLiNa方法对齐中间层能有效提升低资源语言翻译质量，特别是在数据稀缺场景下

Conclusion: TRepLiNa是一种低成本、实用的方法，能显著改善低资源语言的翻译性能
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型的广泛应用，确保其在不同监管环境下可靠处理个人身份信息变得至关重要，特别是在多语言和多样化监管背景下的PII处理。

Method: 采用分阶段的人工参与标注方法，结合语言专业知识和严格的质量保证，利用标注者间一致性指标和根本原因分析来系统发现和解决标注不一致问题。

Result: 框架在试点、训练和生产阶段显著提高了召回率和降低了误报率，生成了适用于监督式LLM微调的高保真数据集。

Conclusion: 迭代的、分析驱动的流水线可以提升多语言PII标注的质量和下游模型的可靠性，同时揭示了多语言PII标注中的常见挑战。
...
<details>
  <summary>Details</summary>
Motivation: 开发标准化的阿育吠陀Prakriti评估工具，为计算智能、阿育吠陀研究和个性化健康分析提供结构化数据平台。

Method: 使用遵循AYUSH/CCRAS指南的24项多选题问卷，通过Google Forms收集数据，自动评分将个体特征映射到dosha特定分数。

Result: 创建了一个包含身体特征、食欲、睡眠模式、能量水平和气质的综合数据集，支持特质分布、相关性和预测建模分析。

Conclusion: 该数据集为Prakriti基础研究、智能健康应用开发提供了有价值的参考平台，促进了阿育吠陀原则的计算应用。
...
<details>
  <summary>Details</summary>
Motivation: 电子健康记录中的非结构化临床数据过多，急诊医生难以快速识别关键信息，需要能在保护患者隐私的前提下离线运行的摘要系统。

Method: 采用双设备架构：Jetson Nano-R负责检索相关病历段落，Jetson Nano-S使用小型语言模型生成结构化摘要，通过轻量级socket通信。摘要包括固定格式的关键发现列表和针对查询的上下文叙事。

Result: 在MIMIC-IV和真实脱敏EHR数据上的初步结果显示，该完全离线系统能在30秒内有效生成有用的摘要。

Conclusion: 该系统证明了在嵌入式设备上实现离线临床摘要的可行性，为保护患者隐私的实时临床决策支持提供了解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型虽然表现出色，但经常产生虚假或捏造的信息（幻觉），这严重影响了模型在需要事实准确性的领域中的可靠性和可信度。

Method: 采用综述研究方法，首先对幻觉类型进行分类，分析整个LLM开发周期中的根本原因，然后构建检测方法和缓解策略的结构化分类法，并评估现有基准和指标。

Result: 建立了全面的幻觉研究框架，包括类型分类、成因分析、检测方法分类、缓解策略分类，以及评估基准和指标的综述。

Conclusion: 幻觉是LLMs面临的关键挑战，需要从数据收集、架构设计到推理的整个开发周期进行系统性解决，未来研究应致力于开发更真实可信的LLMs。
...
<details>
  <summary>Details</summary>
Motivation: 音乐中滥用和性露骨内容急剧增加，特别是Billboard音乐榜，但缺乏验证趋势的研究来制定有效政策，因为这些内容对儿童和青少年行为有有害影响。

Method: 利用深度学习方法和语言模型分析美国Billboard榜过去70年的歌曲歌词，通过情感分析和滥用检测进行纵向研究。

Result: 结果显示1990年后流行音乐中露骨内容显著增加，亵渎、性暗示和其他不当语言的歌词越来越普遍。
...
<details>
  <summary>Details</summary>
Motivation: 复制XRec论文的结果，使用不同的大语言模型（Llama 3）进行评估，并扩展分析专家混合模块嵌入对解释结构的影响。

Method: 基于原始论文的源代码，修改XRec专家混合模块的输入嵌入或删除输出嵌入，使用Llama 3作为评估LLM。

Result: XRec能有效生成个性化解释，其稳定性通过融入协同信息得到提升，但并非在所有指标上都优于基线模型。专家混合嵌入对解释结构有重要影响。

Conclusion: XRec框架有效，专家混合模块嵌入在塑造解释结构方面发挥关键作用，提供了开源实现以增强可访问性。
...
<details>
  <summary>Details</summary>
Motivation: 研究多语言Transformer模型对疑问句形态句法特征的表示能力，评估统计特征与神经表示在捕捉疑问句类型和结构复杂度方面的差异。

Method: 使用QTC数据集（包含7种语言的句子，标注了类型信息和复杂度指标），扩展探针方法到回归标签，比较冻结的Glot500-m模型层间探针与子词TF-IDF基线以及微调模型的性能。

Result: 统计特征在具有显式标记的语言中能有效分类疑问句，而神经探针能更好地捕捉细粒度的结构复杂度模式。

Conclusion: 研究评估了上下文表示何时优于统计基线，以及参数更新是否会减少预训练语言信息的可用性。
...
Result: 在三个掩码语言模型中观察到两种分布下的偏见。在平等分布下实现了近乎完全的缓解，在现实世界设置下减少了30-75%。自回归LLM在平等分布下无偏见，但在现实世界设置下存在显著偏见，Llama Instruct模型实现了50-62%的减少。

Conclusion: 该方法能有效将LLM输出与期望分布对齐，无论是追求平等还是反映现实分布，同时保持模型的语言能力。
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在全球部署，需要创建能够适应全球用户多样化偏好和价值观的多元化系统，而现有数据集不支持奖励模型操控性的受控评估。

Method: 系统生成165,888个偏好对，涵盖传统、世俗理性、生存、自我表达4个价值维度，以及冗长度、可读性、自信度、温暖度4个风格维度，在16种系统提示条件和6种偏好比较场景下评估6个开源和专有模型。

Result: 当提供用户完整的价值观和风格偏好时，最佳模型的准确率低于75%，而仅提供相关风格和价值偏好时准确率超过99%，显示当前奖励模型在识别和适应相关用户信息方面存在局限性。

Conclusion: EVALUESTEER突显了当前奖励模型在适应多样化人类价值观和偏好方面的局限性，为开发可操控的奖励模型提供了具有挑战性的测试平台。
...
<details>
  <summary>Details</summary>
Motivation: 现有大规模多模态模型在视觉问答任务中表现良好，但在需要文化背景知识和日常常识的低资源语言和代表性不足语言中表现不佳，需要创建文化相关的大规模数据集来弥补这一差距。

Method: 开发了EverydayMMQA框架来创建大规模、文化相关的口语和视觉问答数据集，并基于此构建了OASIS多模态数据集，整合语音、图像和文本，涵盖英语和阿拉伯语变体，涉及18个国家。

Result: OASIS数据集包含约92万张图片和1480万QA对，其中370万为口语问题，支持四种输入组合。基准测试了四个闭源模型、三个开源模型和一个微调模型。

Conclusion: EverydayMMQA框架和OASIS数据集为构建具有文化背景理解能力的多模态大语言模型提供了基准和训练数据，将公开发布给研究社区。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM特征的自然语言描述往往模糊、不一致且需要人工重新标注，需要更结构化的描述方法。

Method: 通过结合捕捉语言和语义特征模式的基本元素，以及用于语境化、组合和量化的修饰符，构建语义正则表达式来描述LLM特征。

Result: 语义正则表达式在准确性上匹配自然语言，同时产生更简洁一致的特征描述，支持跨层特征复杂度量化分析，并能帮助用户建立准确的LLM特征激活心智模型。

Conclusion: 语义正则表达式为LLM自动可解释性提供了更精确、结构化的特征描述方法，能够从单个特征洞察扩展到模型级模式分析。
...
<details>
  <summary>Details</summary>
Motivation: 现有的去识别化模型只能隐藏文档中个体的身份信息，但无法解决链接风险问题，即攻击者可能通过提取短语将去识别化文本映射回原始数据源。
...
Conclusion: 该方法成功解决了去识别化中的链接风险问题，在保护隐私的同时维持了文本的语义完整性。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法中，无分类器引导(CFG)能保持语义内容但属性控制效果差，分类器引导(CG)能更好对齐属性但计算成本高且存在分类器泛化问题。

Method: 使用VAE编码器-解码器架构确保重建保真度，结合属性监督训练的潜在扩散模型，仅在训练时注入属性信息。
...
<details>
  <summary>Details</summary>
Motivation: 理解奖励模型的行为，因为RMs作为人类偏好的代理指导语言模型行为，但目前对其行为理解有限，需要研究其社会人口统计偏见。

Method: 建立测量奖励模型观点对齐的框架，调查RMs的社会人口统计偏见程度，探索通过提示引导奖励朝向目标群体偏好的效果。

Result: RMs与多个群体偏好对齐不佳，会系统性地奖励有害刻板印象，提示引导不足以克服这些限制。

Conclusion: 需要在偏好学习中更仔细考虑RM行为，以防止在语言技术中传播不需要的社会偏见。
...
<details>
  <summary>Details</summary>
Motivation: 虚拟实验室为探究式科学学习提供机会，但教师难以调整现有材料或开发定制资源来满足教学需求。

Method: 开发包含四个组件的对齐框架：教学目标理解、实验室理解、问题分类法和TELeR提示分类法，通过教师与LLM的自然语言交互生成教学问题。

Result: 评估了19个开源LLM生成的1100多个问题，结果显示：问题分类法提升认知需求0.29-0.39分，优化提示使80%问题可解析、90%以上符合格式，大型模型表现最佳。

Conclusion: 该框架能有效帮助教师利用LLM生成与教学目标和虚拟实验室内容对齐的高质量教学问题，特别适合大型语言模型。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注简单的文本证据归因，但在金融等真实场景中，归因需要更复杂的支持，包括中间推理步骤和领域知识。

Method: 构建FinLFQA基准，通过人工标注评估三个归因维度，并提供自动评估框架，在八个LLMs上进行了多范式实验。

Result: 细粒度指标能更好区分模型能力，端到端生成与后处理方法表现相当，迭代优化仅在外部反馈指导下有效。

Conclusion: FinLFQA为金融领域的长篇问答提供了更全面的归因评估标准，揭示了当前LLMs在复杂归因任务中的局限性和改进方向。
...
Method: 提出两种训练策略：Multi-Head（每个清单单独分类层）和Masked-Union（通过选择性标签掩码实现共享参数训练）。

Result: Masked-Union方法在参数效率上最强，UniRST在18个单树库基线中超越了16个，展示了单一模型在多语言端到端话语解析中的优势。

Conclusion: UniRST证明了统一的多语言话语解析模型的可行性，Masked-Union策略在参数效率和性能上都表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 现有语言模型在数学基准测试中表现出色，但对语言变体的鲁棒性研究不足。在真实教育环境中，教师会以不同方式表述相同概念，因此语言鲁棒性对于教育应用中的可靠部署至关重要。

Method: 引入MathRobust-LV测试集，通过改变问题的表面细节（如名称、上下文、变量）但保持数值结构和答案不变，来评估模型对语言变体的鲁棒性。专注于高中水平的数学问题，模拟教育评估中的实际应用场景。

Result: 在34个模型上的实验显示，从基准问题到变体问题时准确率下降。小型模型下降9-11%，更强模型也有可测量的性能退化。前沿模型如GPT-5、Gemini-2.5pro相对稳定。

Conclusion: 对语言变体的鲁棒性是语言模型推理能力的基本挑战，暴露了模型在推理方面的脆弱性，即使在数学基准测试看似饱和的情况下，这一维度仍需深入评估。
...
<details>
  <summary>Details</summary>
Motivation: 随着LLM从被动工具向自主智能体的快速转变，这种智能体上下文引入了一类新的固有安全风险，需要系统性的调查和分析。

Method: 通过构建包含应用、威胁和防御三个支柱的综合分类体系，对150多篇相关论文进行了系统性的梳理和分类。

Result: 详细的分析揭示了智能体架构的新兴趋势，同时发现了模型和模态覆盖方面的关键研究空白。

Conclusion: 该调查为理解LLM智能体在网络安全领域的现状提供了全面框架，指出了未来研究的重要方向。
...
<details>
  <summary>Details</summary>
Motivation: 现代ASR系统依赖数据密集型transformer架构，不适用于低资源语言。研究旨在探索ASR在语言文档化中的可行性，特别是针对濒危的澳大利亚原住民语言。

Method: 在Yan-nhangu语言上微调wav2vec2 ASR模型，比较音位标记化和正字法标记化两种策略对性能的影响。

Result: 语言学指导的音位标记化系统相比基线正字法标记化方案，显著降低了词错误率(WER)和字符错误率(CER)。手动校正ASR输出比从头手动转录音频快得多。

Conclusion: ASR可用于低资源语言，音位标记化策略能有效提升性能，ASR在语言文档化流程中具有实用价值。
...
<details>
  <summary>Details</summary>
Motivation: 探索测试时缩放(TTS)在机器翻译任务中的有效性，因为虽然TTS在数学和编程等推理任务中表现出色，但在机器翻译领域的应用尚未充分研究。

Method: 评估12个推理模型在多个领域的机器翻译基准上，研究三种场景：直接翻译、强制推理外推和后期编辑。

Result: 通用模型的直接翻译中TTS效果有限且不稳定；领域特定微调能解锁TTS效果，带来持续改进；强制模型超出自然停止点会降低翻译质量；后期编辑场景中TTS效果显著。

Conclusion: 推理时间计算在机器翻译中的价值不在于用通用模型增强单次翻译，而在于多步骤自校正工作流和任务专用模型等针对性应用。
...
<details>
  <summary>Details</summary>
Motivation: 解决LLMs中模仿学习造成的训练-生成差距，以及强化学习面临的数据瓶颈问题——现有RL数据集规模远小于网络级预训练语料库。

Method: 开发了可扩展的Webscale-RL数据引擎，系统地将大规模预训练文档转化为数百万个多样化、可验证的问答对，构建了包含120万个示例的Webscale-RL数据集。

Result: 在该数据集上训练的模型在多个基准测试中显著优于持续预训练和强数据精炼基线，RL训练效率大幅提升，用少100倍的token达到持续预训练的性能。

Conclusion: 这项工作为将RL扩展到预训练级别提供了可行路径，能够实现更强大和高效的语言模型。
...
<details>
  <summary>Details</summary>
Motivation: 自举预训练有望降低从头训练语言模型的成本，但其在过度训练的基础模型上的有效性尚不明确。

Method: 通过实证研究自举预训练的扩展行为，分析其与基础模型预训练程度的关系。

Result: 发现自举预训练的扩展效率随基础模型预训练程度的增加而对数衰减，可用简单的扩展定律准确建模。

Conclusion: 多阶段预训练策略存在基本权衡：模型预训练越充分，自举带来的额外收益越少，这对高效语言模型训练和过度训练模型重用具有重要启示。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究使用助手语言模型模拟用户对话，但助手模型与真实用户行为存在差异，需要更真实的用户模拟器来评估语言模型在真实场景中的表现

Method: 引入专门训练的用户语言模型(User LMs)，通过后训练使模型能够模拟多轮对话中的人类用户行为

Result: 用户语言模型比现有模拟方法更好地与人类行为对齐，在模拟编程和数学对话时，GPT-4o的性能从74.6%下降到57.4%

Conclusion: 更真实的用户模拟环境揭示了助手模型在多轮对话设置中难以应对用户细微差别的局限性
...
<details>
  <summary>Details</summary>
Motivation: 随着LLMs在自动研究中应用日益复杂，涉及多智能体系统、规划、工具使用等，现有工作流程的扩展和维护变得困难。

Method: 识别自动研究流程的核心组件，提出交互式、可扩展、可控的框架，支持新工具适配和迭代增长。

Result: 提供了开源代码库、交互式Web演示和PyPI Python包，使最先进的自动研究流程对所有研究人员和开发者开放。
...
<details>
  <summary>Details</summary>
Motivation: 随着对话式LLMs的普及，越狱攻击日益严重，现有防御机制难以完全抵抗新型提示技术，需要从模型内部表征角度研究越狱现象。

Method: 分析开源LLM GPT-J和状态空间模型Mamba2的内部表征，比较隐藏层对越狱提示与良性提示的响应模式。

Result: 发现不同层在处理越狱提示时表现出明显不同的行为特征，揭示了层级的响应差异。

Conclusion: 基于内部模型动态的越狱检测和防御是值得进一步研究的有前景方向。
...
Method: 使用中心核对齐、稳定性指标和探测分析等方法，在token和层级上统一分析SSMs和TBMs中的表示传播过程，结合理论分析和参数随机化实验。

Result: 发现TBMs的过平滑源于架构设计，而SSMs的过平滑主要来自训练动态。TBMs快速同质化token表示，后期重新多样化；SSMs早期保留token独特性，深层逐渐同质化。
...
<details>
  <summary>Details</summary>
Motivation: 传统RLHF需要昂贵的人工标注数据，RLAIF也需要收集多样提示和回复并使用外部奖励模型，成本都很高。

Method: SAO让LLM进行角色扮演生成多样提示和回复，然后自我评估进行偏好优化，完全自生成训练数据。

Result: 实验表明SAO有效提升了模型在AlpacaEval~2.0等基准上的对话能力，同时保持了下游客观任务的强性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有AI代理通常使用固定工具，但不同工具在不同任务场景中表现各异。人类通过交互积累工具能力知识来选择最优工具，而现有代理缺乏这种能力。

Method: 提出ToolMem系统，让代理通过先前交互总结工具优缺点并存储在记忆中；推理时检索相关记忆条目，为具体任务选择最佳工具。

Result: 在文本生成和文本到图像生成任务中，ToolMem代理比无记忆通用代理在工具性能预测上分别准确14.8%和28.7%；在工具选择上分别提高21%和24%。

Conclusion: ToolMem通过记忆工具能力使代理能更准确地预测工具性能并选择最优工具，显著提升了AI代理在多样化任务中的表现。
...
<details>
  <summary>Details</summary>
Motivation: 现有对齐数据集多为私有或需要昂贵人工标注，限制了可复现性和可扩展性。即使使用AI反馈，数据质量问题依然存在，且不清楚到底需要多少数据才能将基础模型微调为强大的指令跟随模型。

Method: 引入PiKa数据高效对齐数据集家族，特别是PiKa-SFT数据集仅使用3万个SFT样本，远少于现有数据集。通过在Llama-3-8B-Base和Qwen2.5系列模型上进行微调评估。

Result: PiKa-SFT在AlpacaEval 2.0和Arena-Hard基准测试中表现优异，甚至超越了使用超过1000万专有样本训练的官方Llama-3-8B-Instruct模型。在Qwen2.5系列模型上也取得了一致的性能提升。
...
<details>
  <summary>Details</summary>
Motivation: 解决客服人员在对话过程中需要频繁切换上下文和重复查看历史记录的问题，提升工作效率和用户体验。

Method: 结合微调的Mixtral-8x7B模型进行持续笔记生成，使用DeBERTa分类器过滤琐碎内容，通过代理编辑反馈循环实现模型在线优化和离线重训练。

Result: 在真实部署中，相比批量摘要方法，案例处理时间减少3%（复杂案例最多减少9%），同时获得较高的客服满意度评分。

Conclusion: 增量式摘要结合持续反馈机制能有效提升摘要质量和客服人员的工作效率，具备规模化应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 现有提示工程方法主要针对通用任务优化指令组件，但在机器翻译等任务中，输入组件更为关键，且现有方法通常需要大参数LLM作为辅助工具，适用性有限。

Method: 使用基于反向翻译策略训练的小参数模型进行提示优化，专门针对机器翻译任务设计。
...
<details>
  <summary>Details</summary>
Motivation: 探索LLMs中内容效应的机制，即语义合理性如何影响逻辑有效性判断，这与人类推理中的双过程理论相似但机制不明确。

Method: 研究LLMs内部表示中有效性和合理性的编码方式，使用导向向量验证因果关系，构建解耦向量来减少内容效应。

Result: 发现有效性和合理性在表示几何中线性编码且强对齐，导向向量可因果性地影响判断，对齐程度预测行为内容效应的大小。

Conclusion: LLMs中抽象逻辑概念的表示方式导致内容效应，通过表示干预可以构建更符合逻辑的系统。
...
<details>
  <summary>Details</summary>
Motivation: 现有RL方法在长视野多轮工具使用中存在上下文长度限制、指令跟随性能下降和过高的rollout成本等问题。

Method: 引入基于总结的上下文管理，定期压缩工具使用历史，通过LLM生成的总结保留任务相关信息，在保持紧凑上下文的同时让智能体能够超越固定上下文窗口限制。

Result: 在交互式函数调用和搜索任务上的实验表明，SUPO显著提高了成功率，同时保持相同甚至更低的工作上下文长度。
...
Method: 使用基于LLM的成本高效方法生成语义保持的改写文本，在7个MTEB任务上进行多轮评估，并扩展到3个多语言数据集覆盖10种语言。

Result: 验证了句子编码器性能对词汇空间变化的敏感性，发现较小模型相对较大模型并未受到不成比例的影响，结果在多轮运行中具有统计稳健性。
...
<details>
  <summary>Details</summary>
Motivation: 暴露LLM在信息检索中作为重新排序器的脆弱性，展示其排名行为容易被小的自然语言提示所操控。

Method: 两阶段令牌优化：第一阶段使用贪婪坐标梯度结合可读性评分筛选候选令牌；第二阶段通过基于熵的动态加权方案评估候选令牌，使用温度控制采样选择令牌。

Result: 实验表明RAF能显著提升目标项目的排名，使用自然语言且比现有方法更具鲁棒性。

Conclusion: LLM基于重新排序本质上容易受到对抗性操纵，这对现代检索系统的可信性和鲁棒性提出了新挑战。
...
<details>
  <summary>Details</summary>
Motivation: 保护大语言模型的知识产权至关重要，需要可靠的方法来判断可疑模型是否源自现有基础模型，但复杂的后训练过程给识别带来了巨大挑战。

Method: 使用权重矩阵指纹方法，结合线性分配问题和无偏中心化核对齐相似度来抵消参数操作的影响，提供高度鲁棒的相似度度量。

Result: 在60个正样本和90个负样本对上的测试显示，该方法对所有六种后训练类别都具有卓越的鲁棒性，假阳性风险接近零，所有分类指标均达到完美分数。
...
Method: 基于迭代向量更新：构建基于代表性文本的稀疏向量，然后通过LLM指导迭代优化，无需假设先验的聚类知识或标签。

Result: 在多样化数据集和小型LLM上的实验表明，该方法与使用对比学习的最先进方法相比，取得了相当或更好的结果，且具有模型无关性、可扩展性和计算效率。

Conclusion: 该方法的低资源需求、适应性和可扩展性使其比现有聚类方法更符合真实世界场景的需求。
...
<details>
  <summary>Details</summary>
Motivation: 现有语法错误纠正评估指标主要基于编辑操作且以英语为中心，依赖于系统和参考编辑之间的严格对齐，限制了在多语言和生成式设置中的适用性。

Method: 引入基于流畅度的多参考评估框架，将n-gram相似度建模为多个合法修正的聚合问题，实例化了四种聚合策略：选择最佳、简单平均、加权平均和合并计数。

Result: 在捷克语、爱沙尼亚语、乌克兰语和中文语料库上的实证结果表明，这些策略捕捉了流畅性和覆盖度的互补方面。

Conclusion: 该框架将多参考评估统一为一种原则性的、以流畅度为导向的方法，既包含了语言多样性，又不会惩罚合法的变体。
...
Result: 通过叠加部署策略和轻量级调节，能够在减少计算量的同时保持推理能力。
...
Method: 通过自动形式化接口，自动识别自然语言问题中的形式推理策略，并动态选择和集成专门的逻辑求解器。

Result: LLMs在预测必要的形式推理策略方面准确率超过90%；框架性能优于GPT-4o和DeepSeek-V3.1，分别高出27%和6%；自适应推理还能提升纯LLM方法，在GPT-4o的零样本、CoT和符号CoT设置中分别获得10%、5%和6%的提升。
...
TL;DR: 该论文系统研究了LLM知识物化过程，通过miniGPTKBs分析终止性、可重现性和鲁棒性，发现核心知识提取可靠但存在模型依赖性和可重现性混合等问题。


<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型编码了大量事实知识，但如何系统化测量和提取这些知识仍具挑战性，特别是关于提取过程能否终止、输出是否可重现以及对变化的鲁棒性等关键问题。

Method: 使用miniGPTKBs（领域特定、可处理的子爬取）方法，通过四种变体（种子、语言、随机性、模型）和三个示例领域（历史、娱乐、金融），分析终止性、可重现性和鲁棒性三个维度的指标。

Result: 研究发现：(i) 终止率较高但依赖模型；(ii) 可重现性结果混合；(iii) 鲁棒性因扰动类型而异：对种子和温度的鲁棒性高，对语言和模型的鲁棒性较低。

Conclusion: LLM知识物化能够可靠地提取核心知识，但也揭示了重要局限性，特别是在模型依赖性和某些扰动类型下的鲁棒性方面。
...
TL;DR: FURINA-Builder是一个自动构建可定制角色扮演基准的多智能体协作管道，FURINA-Bench是基于此构建的综合基准。研究发现推理LLMs在角色扮演中存在性能与幻觉的权衡。


<details>
  <summary>Details</summary>
Motivation: 现有角色扮演基准因范围狭窄、交互范式过时和适应性有限而快速过时，需要更灵活、可扩展的评估方法。

Method: FURINA-Builder通过多智能体协作模拟角色间对话，使用LLM法官选择评估维度并调整测试角色的响应，构建可定制的角色扮演基准。

Result: o3和DeepSeek-R1分别在英文和中文角色扮演任务中表现最佳；已建立角色优于合成角色；模型规模不单调减少幻觉；推理LLMs存在性能与幻觉的权衡。

Conclusion: FURINA-Builder有效解决了角色扮演基准构建的挑战，揭示了推理能力在提升性能的同时会增加幻觉的重要权衡关系。
...
TL;DR: PAN 2025生成性抄袭检测任务旨在识别科学文章中的自动生成文本抄袭并匹配其来源。研究创建了使用Llama、DeepSeek-R1和Mistral三种大语言模型的新型大规模自动生成抄袭数据集，评估了参与者方法和基线方法的表现。


<details>
  <summary>Details</summary>
Motivation: 应对自动生成文本抄袭检测的挑战，创建大规模数据集来评估现有方法的有效性，并检验这些方法在不同数据集上的泛化能力。

Method: 使用三种大语言模型（Llama、DeepSeek-R1、Mistral）创建自动生成抄袭数据集，比较参与者提交的方法与四个基线方法，并在PAN 2015数据集上评估方法的鲁棒性。

Result: 当前方法多样性不足，基于嵌入向量的简单语义相似度方法在2025数据集上表现良好（召回率0.8，精确率0.5），但在2015数据集上表现显著下降，表明泛化能力不足。

Conclusion: 当前生成性抄袭检测方法在特定数据集上表现良好但缺乏泛化性，需要开发更具鲁棒性的方法来应对不同来源的自动生成抄袭。
...
TL;DR: 该论文研究了通过集成多种电路定位方法来提高大型语言模型中任务行为相关子网络定位性能的方法，提出了并行和序列两种集成策略，并在MIB基准测试中取得了显著改进。


<details>
  <summary>Details</summary>
Motivation: 现有的单一电路定位方法在大型语言模型中识别特定任务行为的子网络时存在局限性，研究者希望通过集成多个方法来提升定位的准确性和鲁棒性。

Method: 提出了两种集成方法：并行集成（通过平均、最小或最大值组合不同方法的边属性分数）和序列集成（使用EAP-IG作为预热启动更精确但更昂贵的边剪枝方法）。

Result: 两种集成方法都在基准测试指标上取得了显著提升，其中并行集成各种方法（包括序列集成）获得了最佳结果，在BlackboxNLP 2025 MIB共享任务中优于官方基线。

Conclusion: 集成多个电路定位方法能够有效提高大型语言模型中任务行为子网络的定位精度，为机械可解释性研究提供了更可靠的电路识别方法。
...
TL;DR: MTR是一个模拟优先的训练框架，通过多智能体架构生成工具接口和推理轨迹，使用两阶段训练（SFT+GRPO）来学习工具增强推理，无需实时API访问。


<details>
  <summary>Details</summary>
Motivation: 现有工具增强语言模型依赖实时API访问，存在可扩展性和可靠性问题，需要一种不依赖实时交互的训练方法。

Method: 采用多智能体架构：ToolMaker生成工具接口，AutoAgent产生推理序列，ToolActor模拟响应。两阶段训练：SFT学习轨迹语法，GRPO优化策略。

Result: 在四个多跳QA基准测试中，MTR达到与实时API系统相当的精确匹配分数，在推理密集型任务上表现优异。

Conclusion: 有效的工具推理可以通过结构化轨迹学习，无需实时交互，为工具增强语言模型提供了更可扩展和可靠的训练方案。
...
<details>
  <summary>Details</summary>
Motivation: 尽管中间训练阶段在先进系统中被广泛使用，但之前缺乏将其作为统一范式的系统性调查，本文旨在填补这一空白。

Method: 通过梯度噪声尺度、信息瓶颈和课程学习等理论框架分析中间训练的有效性，并构建了涵盖数据分布、学习率调度和长上下文扩展的分类法。

Result: 提出了首个LLM中间训练分类法，整理了实践见解、评估基准和性能提升数据，为模型间的结构化比较提供了基础。

Conclusion: 中间训练阶段通过多阶段退火式训练有效缓解了噪声令牌的收益递减、稳定了收敛并扩展了模型能力，为未来研究提供了系统框架和开放挑战方向。
...
TL;DR: 本文构建了一个大规模挑战集，专门用于检测自动质量评估指标在评估包含性别模糊职业术语的翻译时是否存在性别偏见。
...
Result: 构建了一个规模大、覆盖面广、完全平行设计的数据集，能够按职业进行细粒度偏见分析，并支持跨语言系统比较。

Conclusion: 无偏见的QE指标应该为两个版本分配相等或接近相等的分数，该数据集为检测和量化QE指标中的性别偏见提供了有力工具。
...
TL;DR: 本文提出SID方法，利用LLM的自信号（模型级置信度和词级语义焦点）来指导多LLM辩论过程，提高准确性和效率


<details>
  <summary>Details</summary>
Motivation: 现有多LLM辩论方法主要依赖外部结构，忽略了生成过程中的自信号（如token logits和注意力），导致计算冗余和性能下降

Method: 引入SID方法，利用模型级置信度实现高置信度代理提前退出，基于注意力机制压缩冗余辩论内容

Result: 在多个挑战性基准测试中，SID方法在准确性和token消耗方面均优于现有MAD技术
...
TL;DR: OpenJAI-v1.0是基于Qwen3-14B开发的开源泰英双语大语言模型，在指令跟随、长上下文理解和工具使用等实际任务上表现优异，超越其他开源泰语模型。


<details>
  <summary>Details</summary>
Motivation: 为泰语AI社区提供高质量的开源NLP资源，解决泰语大语言模型的稀缺问题，提升在实际应用任务上的性能。

Method: 从Qwen3-14B模型出发，通过精心策划的数据集在三个关键用例上进行优化：指令跟随、长上下文理解和工具使用。

Result: 评估结果显示OpenJAI-v1.0在基础模型能力上有所提升，在多样化基准测试中超越其他领先的开源泰语模型，同时避免了灾难性遗忘。

Conclusion: OpenJAI-v1.0作为开源泰英双语模型，为泰语AI社区提供了有价值的替代NLP资源，在实际应用任务上表现出色。
...
TL;DR: 本文研究大语言模型在上下文感知翻译中的语篇现象处理能力，提出质量感知解码方法以有效提取模型中的语篇知识，并证明该方法优于其他解码策略。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在机器翻译中表现出色，但在处理语篇层面的代词解析和词汇连贯性等语篇现象时仍存在困难，需要研究如何更好地利用其内在的语篇知识。

Method: 提出质量感知解码方法，通过有效提取大语言模型中编码的语篇知识来改进上下文感知翻译性能。

Result: 质量感知解码方法在语篇现象处理上优于其他解码方法，能够增强翻译的语义丰富度，并更符合人类偏好。

Conclusion: 大语言模型中确实编码了语篇知识，通过质量感知解码可以有效提取这些知识，显著提升上下文感知翻译的质量和人类偏好度。
...
TL;DR: 提出了λ-GRPO方法，通过可学习的参数λ自适应控制token级权重，解决了GRPO中的长度偏差问题，在数学推理基准上相比GRPO和DAPO取得了稳定提升。


<details>
  <summary>Details</summary>
Motivation: 现有的RLVR方法如GRPO存在长度偏差问题，即相同优势值被均匀分配给响应中的所有token，导致较长响应在梯度更新中贡献不成比例。现有变体方法如DAPO和Dr.GRPO虽然修改了token级损失聚合，但仍缺乏可解释性。
...
Conclusion: 学习token偏好是有效且实用的方法，λ-GRPO在多个数学推理基准上一致优于vanilla GRPO和DAPO。
...
Result: MeXtract在MOLE基准测试中达到最先进性能，微调后的模型不仅精度高，还能有效迁移到未见过的模式，展示了鲁棒性和适应性。

Conclusion: MeXtract为科学文献元数据提取提供了高效解决方案，具有良好的泛化能力，所有代码、数据集和模型已开源供研究社区使用。
...
TL;DR: 提出了Long-RewardBench基准和LongRM方法，用于解决长上下文奖励模型在保持上下文一致性方面的脆弱性问题。


<details>
  <summary>Details</summary>
Motivation: 现实应用中涉及长历史轨迹（如LLM智能体）时，需要评估模型响应不仅质量高，还要与上下文保持一致。现有奖励模型局限于短上下文设置，忽视了长上下文-响应一致性的关键维度。

Method: 提出多阶段训练策略，将任意模型扩展为鲁棒的长上下文奖励模型（LongRMs），包括在Long-RewardBench基准上进行成对比较和最佳选择任务评估。

Result: 实验表明该方法显著提升了长上下文评估性能，同时保持了强大的短上下文能力。8B的LongRM超越了70B规模的基线模型，性能与专有的Gemini 2.5 Pro模型相当。

Conclusion: 多阶段训练策略能有效构建鲁棒的长上下文奖励模型，解决了现有模型在长上下文场景中的脆弱性问题，为LLM与人类偏好的对齐提供了重要改进。
...
TL;DR: SHANKS是一个推理框架，让语音语言模型在用户说话时就能生成内部推理，实现边听边思考，降低响应延迟，提升实时交互能力。
...
Method: 将输入语音流式分块处理，每收到一个语音块就基于之前所有语音和推理生成内部推理，决定是否打断用户或调用工具完成任务。
...
TL;DR: Open ASR Leaderboard是一个可复现的语音识别基准测试平台，比较了60多个开源和专有系统在11个数据集上的表现，包含多语言和长文本赛道，标准化了文本归一化并同时报告WER和RTFx指标。


<details>
  <summary>Details</summary>
Motivation: 当前ASR评估主要集中于短文本英语，且很少报告效率指标，需要建立全面、可复现的评估基准。

Method: 创建包含多语言和长文本赛道的交互式排行榜，标准化文本归一化，使用WER和RTFx指标进行准确性和效率的公平比较。

Result: 英语转录中Conformer编码器+LLM解码器获得最佳平均WER但速度较慢，CTC和TDT解码器RTFx更好适合长文本和离线使用；Whisper编码器英语微调提升准确性但牺牲多语言覆盖。
...
TL;DR: 使用LLMs生成符合学生兴趣和教育标准的数学应用题，通过专家-LLM联合评估方法开发教师标注数据集，训练出的模型在生成质量上优于现有模型，学生测试显示定制化题目表现良好且更受欢迎。


<details>
  <summary>Details</summary>
Motivation: 数学应用题是重要的K-12教育工具，但教师因班级规模大和工作负担重而难以个性化定制题目。LLMs有潜力支持数学教育，生成符合学生兴趣和课程标准的定制化题目。

Method: 采用专家-LLM联合评估方法，评估了11,000多个由开源和闭源LLMs生成的数学应用题，建立了首个教师标注的教育标准对齐数据集，并基于此训练了12B和30B的开源模型。

Result: 训练出的12B模型性能与更大规模开源模型相当；使用教师标注数据训练的文本分类器使30B模型无需训练即可超越现有闭源基线；生成的题目比现有模型更接近人工编写题目；学生测试显示在定制化题目上表现与人工题目相当但更受欢迎。

Conclusion: LLMs能够有效生成符合教育标准和学生兴趣的数学应用题，通过专家标注数据训练的模型在生成质量和学生接受度方面表现优异，为个性化数学教育提供了可行方案。
...
TL;DR: 提出摘要增强分块(SAC)方法，通过在文本块中添加文档级合成摘要来减少法律文档检索中的文档级检索不匹配(DRM)问题，提高RAG系统的可靠性。


<details>
  <summary>Details</summary>
Motivation: 法律领域的大规模结构化相似文档数据库常导致检索系统失败，特别是文档级检索不匹配(DRM)问题，即检索器从完全错误的源文档中选择信息。

Method: 提出摘要增强分块(SAC)方法，在标准分块过程中为每个文本块添加文档级合成摘要，注入关键全局上下文信息。

Result: 在多样化法律信息检索任务上的实验表明，SAC显著减少了DRM，并因此提高了文本级检索的精确率和召回率。通用摘要策略优于包含法律专家领域知识的方法。
...
TL;DR: 提出了一个人机协作的数据生成流程，创建了包含22.5K指令调优数据和100K对齐数据的多语言数据集，覆盖10种印度语言，旨在解决现有开源数据集在印度语言上的覆盖不足问题。


<details>
  <summary>Details</summary>
Motivation: 现有开源数据集缺乏多语言覆盖、文化基础，在印度语言上存在显著的任务多样性差距，这限制了大型语言模型在印度语言上的有效性。

Method: 采用人机协作流程，结合翻译和合成扩展来生成可靠的印度语言后训练数据，创建了两个数据集Pragyaan-IT和Pragyaan-Align，覆盖10种印度语言、13个主要类别和56个子类别。

Result: 成功构建了包含22.5K指令调优样本和100K对齐样本的数据集，涵盖57个多样化数据集，在任务多样性、多轮对话、指令保真度、安全对齐和文化细微差别等方面表现出色。

Conclusion: 该数据集协议为构建更具包容性和有效的多语言大语言模型提供了基础，特别关注了常被忽视的维度，如文化细微差别的保留。
...
TL;DR: 提出Native Hybrid Attention (NHA)，一种结合线性注意力和完整注意力的混合架构，通过单一softmax操作实现长短期上下文整合，在保持效率的同时提升长上下文召回准确率。


<details>
  <summary>Details</summary>
Motivation: Transformer存在二次复杂度问题，线性注意力虽然效率高但在长上下文召回准确率上表现不佳，需要一种能兼顾效率和准确率的混合解决方案。

Method: NHA采用线性RNN更新长时上下文键值槽，结合滑动窗口的短时token，通过单一softmax注意力操作实现上下文依赖的权重分配，无需额外融合参数。

Result: NHA在召回密集型和常识推理任务上超越Transformer和其他混合基线方法，预训练LLM通过NHA结构混合后能在保持竞争力的准确率同时获得显著效率提升。

Conclusion: NHA提供了一种简单有效的混合注意力机制，通过单一超参数平滑调整线性与完整注意力的平衡，在效率和准确率之间取得了良好权衡。
...
TL;DR: 对GPT-4.1的1亿条信念分析显示，其事实知识与标准知识库差异显著，准确性低于先前基准，存在严重的不一致、模糊和幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 理解前沿LLM的事实知识特性，现有分析多基于有偏样本，需要更全面的评估。

Method: 基于GPTKB v1.5数据集，对GPT-4.1的1亿条递归获取的信念进行深入分析。

Result: 模型事实知识与标准知识库差异显著，准确性低于预期，存在不一致性、模糊性和幻觉等主要问题。

Conclusion: LLM的事实知识存在严重缺陷，揭示了未来关于事实知识研究的机遇。
...
TL;DR: 这篇论文对代码转换（CSW）感知的大型语言模型研究进行了首次全面分析，涵盖了5个研究领域、12个NLP任务、30多个数据集和80多种语言，指出了当前LLM在多语言混合输入方面面临的挑战，并提出了未来发展路线图。


<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型快速发展，但代码转换（语言和脚本在单个话语中的交替）仍然是多语言NLP的基本挑战。大多数LLM在处理混合语言输入、有限的数据集和评估偏见方面仍然困难，阻碍了在多语言社会中的部署。

Method: 通过系统综述方法，对现有研究进行分类分析，涵盖架构、训练策略和评估方法三个维度，整理和分析了大量相关研究、数据集和语言资源。

Result: 提供了对CSW感知LLM研究的全面分析框架，识别了当前研究进展和持续存在的挑战，建立了资源集合供研究社区使用。

Conclusion: 提出了未来发展路线图，强调需要包容性数据集、公平评估和基于语言学的模型，以实现真正的多语言智能。
...
TL;DR: Search-R3是一个新颖框架，通过让大语言模型在推理过程中直接生成搜索嵌入，将LLMs的推理能力与检索任务相结合。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型具有强大的自然语言理解能力，但在检索任务中尚未得到充分利用。作者希望开发一个框架来弥补这一局限性。

Method: 采用三步机制：1) 监督学习阶段训练模型生成高质量嵌入；2) 强化学习方法优化嵌入生成和推理过程；3) 专门的RL环境处理动态嵌入表示，避免每次迭代重新编码整个语料库。

Result: 在多个基准测试上的广泛评估表明，Search-R3通过统一推理和嵌入生成过程，显著优于先前的方法。
...
TL;DR: 研究显示，辛克莱广播集团收购地方新闻台后，报道内容从地方议题转向全国性政治话题，增加了极化内容的报道频率。


<details>
  <summary>Details</summary>
Motivation: 地方新闻台通常被认为是可靠的非政治化信息来源，辛克莱集团近年收购了大量地方新闻台，需要研究这种收购对报道内容的影响。

Method: 使用计算方法分析地方新闻台在被辛克莱收购前后的网络内容变化，并与全国性媒体进行比较。

Result: 地方新闻台被收购后更频繁报道全国性新闻而减少地方议题，对极化全国话题的报道增加。

Conclusion: 辛克莱收购改变了地方新闻台的报道重点，使其从服务地方社区转向报道更多全国性政治内容。
...
TL;DR: ITEM是一个大规模基准测试，系统评估了26个自动指标在6种主要印度语言中与人类判断的一致性，发现基于LLM的评估器与人类判断最一致，离群值对指标-人类一致性有显著影响，不同指标在内容保真度和流畅性评估方面表现不同。
...
Method: 引入ITEM基准测试，通过细粒度注释系统评估26个自动指标与人类判断在6种印度语言中的一致性，涵盖与人类判断的一致性、对离群值的敏感性、语言特定可靠性、指标间相关性和对受控扰动的鲁棒性。

Result: 四个主要发现：(1)基于LLM的评估器在片段和系统级别与人类判断最一致；(2)离群值对指标-人类一致性有显著影响；(3)在文本摘要中指标更能捕捉内容保真度，在机器翻译中更能反映流畅性；(4)不同指标对扰动的鲁棒性和敏感性不同。
...
<details>
  <summary>Details</summary>
Motivation: 低资源语言如卢森堡语缺乏高质量的指令数据集，传统机器翻译方法存在语义不对齐和文化不准确的问题。
...
TL;DR: 提出了LocalLeap方法，通过局部确定性传播和渐进空间一致性衰减原则，在扩散大语言模型中实现自适应并行解码，显著提升推理效率。


<details>
  <summary>Details</summary>
Motivation: 现有开源扩散大语言模型在质量与速度之间存在权衡问题，保守的采样策略导致推理效率低下，存在延迟解码现象。

Method: 基于局部确定性传播和渐进空间一致性衰减两个经验原则，识别锚点并在有界邻域内执行局部松弛并行解码，实现早期确定令牌的承诺。

Result: 在各种基准测试中，LocalLeap实现了6.94倍的吞吐量提升，将解码步骤减少至原始需求的14.2%，且性能影响可忽略不计。

Conclusion: LocalLeap是一种无需训练的自适应并行解码策略，能有效解决扩散大语言模型的延迟解码问题，显著提升推理效率而不影响输出质量。
...
TL;DR: 提出VITALERRORS基准和VITAL指标，用于更敏感地检测LLM响应中关键信息的错误，相比现有方法能更可靠地识别重要信息的遗漏或错误。


<details>
  <summary>Details</summary>
Motivation: 现有LLM事实性评估方法将所有声明视为同等重要，导致关键信息错误时评估不准确，需要更敏感的方法来检测关键信息的错误。

Method: 构建包含6,733个查询的VITALERRORS基准数据集，通过最小化修改LLM响应来遗漏或伪造关键信息，并开发VITAL指标来结合声明与查询的相关性和重要性。

Result: 实验表明现有评估指标对关键信息错误不敏感，而VITAL指标能更可靠地检测关键信息错误。
...
TL;DR: 提出了一种基于LLM增强的检索增强框架，用于讽刺感知的语音合成，结合语义嵌入和韵律示例来生成更自然的讽刺语音。
...
Method: 使用LoRA微调的LLaMA 3获取语义嵌入，结合RAG模块检索韵律示例，在VITS骨干网络中集成双重条件生成。

Result: 实验表明该方法在客观指标和主观评估中均优于基线，在语音自然度、讽刺表达性和下游讽刺检测方面均有提升。

Conclusion: 该框架成功实现了更自然和上下文适当的讽刺语音合成，为处理微妙语言表达提供了有效解决方案。
...
TL;DR: TALENT是一个轻量级框架，通过结合OCR文本和自然语言叙述的双重表示，将表格VQA重构为以LLM为中心的多模态推理任务，显著降低了计算成本。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在表格VQA中往往忽略细节且计算成本高，而小VLM+LLM的方法中结构化表示对LLM不友好且错误率高。

Method: 使用小VLM生成OCR文本和自然语言叙述，然后结合问题输入LLM进行推理，将VLM作为感知-叙述模块而非整体求解器。

Result: 在公共数据集和ReTabVQA上，TALENT使小VLM-LLM组合能够匹配或超越单个大型VLM，计算成本显著降低。

Conclusion: TALENT通过双重表示和LLM中心化方法，为表格VQA提供了高效轻量的解决方案，并在更具挑战性的数据集上验证了其有效性。
...
TL;DR: 提出了一种基于语言模型和元学习的人类变异性建模系统，在LeWiDi竞赛中获胜，并通过消融研究验证了系统组件的重要性。
...
Method: 利用语言模型的上下文学习能力，采用两步元学习训练：1）在多个需要上下文学习的数据集上进行后训练；2）通过上下文元学习使模型适应特定数据分布。

Result: 在LeWiDi竞赛中两个任务均获胜，消融研究表明：上下文包含评分者示例至关重要，数据集特定微调在大数据集上有帮助，在其他上下文数据集上的后训练在一个竞赛数据集上有帮助，性能随模型规模提升。

Conclusion: 所提出的系统能有效建模人类变异性，上下文学习和元学习是关键，模型规模和特定训练策略对性能有重要影响。
...
TL;DR: TRIM是一种基于前向传播的token级核心集选择方法，通过注意力指纹匹配任务特征，在显著降低计算成本的同时，在指令调优任务上超越现有方法并接近全数据微调性能。


<details>
  <summary>Details</summary>
Motivation: 现有核心集选择方法依赖梯度等样本级信号，计算成本高且忽略了细粒度特征，需要一种更高效、更敏感的任务特征识别方法。

Method: 提出TRIM框架，基于前向传播，通过注意力机制生成token级指纹，匹配少量目标样本的表征模式来识别任务关键特征。

Result: TRIM选择的核心集在下游任务上比现有最优方法提升高达9%，在某些设置下甚至超越全数据微调性能，同时计算成本显著降低。
...
TL;DR: 本研究系统比较了人类与大型语言模型在七种复杂语言结构上的句子理解能力，发现LLMs在花园路径句上表现尤其困难，模型性能与参数数量相关，揭示了人类与LLM在语言理解上的异同。


<details>
  <summary>Details</summary>
Motivation: 探究大型语言模型是否像人类一样经历语言处理困难，比较人类与LLM在复杂语言结构上的理解表现。

Method: 在统一实验框架下收集人类和五个系列最先进LLMs的句子理解数据，涵盖不同规模和训练过程，测试七种挑战性语言结构及其基线对照。

Result: LLMs在目标结构上总体表现不佳，特别是在花园路径句上（GPT-5仅46.8%准确率），而在非花园路径结构上表现良好（GPT-5达93.7%）。模型与人类的结构难度排序相关性随参数数量增加而提高。

Conclusion: 研究揭示了人类与LLM在句子理解上的趋同与分歧，为理解人类与AI语言处理的相似性提供了新见解。
...
TL;DR: 提出RHC框架，将层次文本分类重新定义为逐步推理任务，通过两阶段训练LLM来提升专利分类性能，在准确率和可解释性方面均有显著改进


<details>
  <summary>Details</summary>
Motivation: 传统方法只输出扁平标签集，无法提供预测背后的推理过程，缺乏可解释性

Method: RHC框架将HTC重构为逐步推理任务，采用两阶段训练：冷启动阶段对齐CoT推理格式，强化学习阶段增强多步推理能力

Result: RHC在准确率和宏观F1上比基线方法提升约3%，在多个HTC基准测试中达到最先进性能

Conclusion: RHC在专利分类等困难HTC场景中表现出有效性、可解释性、可扩展性和广泛适用性
...
TL;DR: 该研究对开源数学推理数据集和合成技术进行了综合分析，发现在实际应用中，提高数据质量（如结构化格式、从强模型蒸馏）比单纯扩大数据量更有效。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型的推理能力对下游任务至关重要，但依赖于训练数据的质量。尽管已有多种数据构建方法，但它们在真实世界流水线中的实用性仍未充分探索。

Method: 在统一设计的训练和部署流水线下，对开源数据集和数据合成技术进行综合分析，提炼有效的数据选择策略。

Result: 研究发现，将数据组织成更易解释的格式，或从更强的模型中进行知识蒸馏，通常比单纯扩大数据规模更有效。

Conclusion: 该研究为整合训练数据以增强LLM能力提供了实用指导，支持成本效益高的数据管理和可扩展的模型增强，并激发关于在真实世界推理任务中如何平衡"更多数据"与"更好数据"的进一步研究。
...
TL;DR: 提出了首个护理专业大语言模型NurseLLM，专注于多选题任务，通过多阶段数据生成管道构建大规模护理数据集，在多个基准测试中优于同类通用和医疗专业模型。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型在护理等专业领域的潜力尚未充分探索，需要专门针对护理场景的模型来提升医疗系统的专业能力。

Method: 开发多阶段数据生成管道构建大规模护理多选题数据集，训练专门的NurseLLM模型，并建立多个护理基准测试进行严格评估。

Result: NurseLLM在不同基准测试中优于同类规模的通用和医疗专业大语言模型，证明了护理领域专门化模型的重要性。

Conclusion: 护理专业大语言模型具有重要价值，推理和多智能体协作系统在护理领域展现出未来研究和应用的潜力。
...
TL;DR: 提出了一个系统框架来量化LLM心理测量评估中的数据污染问题，发现主要心理测量量表存在严重的数据污染，模型不仅能记忆题目还能调整回答以达到特定目标分数。


<details>
  <summary>Details</summary>
Motivation: 现有研究使用心理测量问卷评估LLM的高阶心理构念，但存在数据污染威胁评估可靠性的担忧，缺乏对此问题的系统量化研究。

Method: 提出了一个系统框架来测量LLM心理测量评估中的数据污染，评估三个方面：(1)题目记忆，(2)评估记忆，(3)目标分数匹配。应用于21个主要模型家族和4个广泛使用的心理测量量表。

Result: 发现流行量表如BFI-44和PVQ-40存在强烈污染，模型不仅能记忆题目，还能调整回答以达到特定目标分数。

Conclusion: 心理测量评估中的数据污染问题严重，威胁LLM心理构念评估的可靠性，需要更严谨的评估方法。
...
TL;DR: 本文提出了CARPAS任务，旨在根据文档内容动态调整预定义方面以改进基于方面的摘要生成。研究发现LLMs倾向于预测过多方面导致摘要过长，提出通过预测相关方面数量来指导LLMs，显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现实场景中预定义方面可能不完整、不相关或缺失，用户期望系统能根据实际内容自适应地精炼或过滤这些方面。

Method: 构建三个新数据集，使用LLMs和四种提示策略，提出预测相关方面数量的子任务作为指导，减少推理难度。

Result: 提出的方法在所有数据集上显著提升性能，发现LLMs在请求方面数量与其自身估计不同时表现出遵从性。

Conclusion: 预测相关方面数量能有效指导LLMs生成更相关和简洁的摘要，为LLMs在类似现实应用中的部署提供了重要见解。
...
TL;DR: 本文通过更广泛的语言和不可能扰动测试，发现GPT-2在大多数情况下学习自然语言和不可能语言同样容易，表明LLMs不共享人类塑造语言类型学的先天偏见。


<details>
  <summary>Details</summary>
Motivation: 探究大型语言模型是否像人类一样能够区分人类可能的语言和不可能的语言，这关系到LLMs与人类是否共享相同的先天学习偏见。

Method: 使用更广泛的语言集和不可能扰动，比较GPT-2在自然语言和不可能语言上的学习曲线，并测试模型是否能在整体上区分这两类语言。

Result: GPT-2在大多数情况下学习自然语言和不可能语言同样容易，且无法系统地区分可能和不可能语言集合。

Conclusion: LLMs不共享人类塑造语言类型学的先天偏见，与人类的学习机制存在根本差异。
...
TL;DR: 开发了针对乌干达语言优化的Sunflower 14B和32B模型，基于Qwen 3架构，在乌干达大多数语言中具有最先进的理解能力。


<details>
  <summary>Details</summary>
Motivation: 非洲有2000多种语言，但现有LLM主要支持使用人数最多的语言，导致对分散语言的支持不均衡。主张采用区域聚焦方法更高效地解决语言技术差距。

Method: 以乌干达为案例研究，基于Qwen 3开发了Sunflower 14B和32B模型，专门针对乌干达的高语言多样性进行优化。

Result: 开发的模型在乌干达大多数语言中表现出最先进的理解能力，是开源模型，可用于减少重要实际应用中的语言障碍。

Conclusion: 区域聚焦方法比零散支持不同语言更有效，Sunflower模型为乌干达语言技术发展提供了实用解决方案。
...
TL;DR: 提出一种无需训练的方法，通过识别和操纵大语言模型中间层到输出层的稀疏维度，实现跨语言生成控制，仅需少量数据即可切换输出语言并保持语义内容。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在英语数据上训练，但表现出强大的多语言能力。研究发现模型将多语言内容映射为英语对齐表示，然后在输出层投影回目标语言。我们假设这种跨语言转换由少量稀疏维度控制。

Method: 提出简单、无需训练的方法，仅需50句平行或单语数据，识别中间层到输出层的一致索引处的稀疏维度，并通过干预这些维度来控制输出语言。

Result: 实验表明这些维度具有可解释性，干预能切换输出语言同时保持语义内容，性能优于先前的基于神经元的方法，且成本显著降低。

Conclusion: 大语言模型的跨语言能力由少量稀疏维度控制，通过简单干预这些维度可实现高效的多语言生成控制。
...
TL;DR: 该论文研究了低资源非洲语言ASR系统的实际部署需求，通过实验确定了Whisper模型在Kinyarwanda和Kikuyu语言上的最小数据量需求（50小时可达WER<13%）和主要错误模式（38.6%高错误率由数据质量问题导致）。
...
Method: 在两个班图语（Kinyarwanda和Kikuyu）上进行全面实验：系统数据缩放分析（1-1400小时训练数据）和详细错误特征分析（270小时训练数据）。

Result: 实验显示：50小时训练数据可实现WER<13%的实用性能，200小时可达WER<10%；38.6%的高错误案例由数据质量问题（特别是噪声真实转录）导致。
...
TL;DR: 提出了一个高效的小语言模型预训练框架，通过结构稀疏子网络初始化、进化搜索和知识蒸馏三个互补技术，显著提升SLM预训练效率
...
Method: 1) 识别结构稀疏子网络初始化；2) 使用进化搜索自动发现高质量子网络初始化；3) 应用大模型知识蒸馏加速训练

Result: 最佳模型在验证困惑度上匹配了可比Pythia SLM，同时减少了9.2倍的预训练token需求
...
TL;DR: 提出了Customer-R1方法，使用强化学习在在线购物环境中模拟个性化的分步用户行为，通过基于明确用户画像的策略优化，在行为预测和个性化模拟方面显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要学习群体层面的策略，没有基于用户画像进行个性化建模，导致模拟结果过于通用而非个性化。

Method: 提出Customer-R1方法，基于强化学习，策略明确考虑用户画像，通过动作正确性奖励信号优化下一步推理和动作生成。

Result: 在OPeRA数据集上的实验表明，Customer-R1在下一步动作预测任务中显著优于提示和SFT基线方法，且能更好地匹配用户动作分布。

Conclusion: Customer-R1能够实现更高保真度的个性化行为模拟，证明了基于用户画像的强化学习方法在个性化用户行为模拟中的有效性。
...
TL;DR: 本文提出了一个基于顶级经济金融期刊中因果识别关系构建的新基准，用于评估大语言模型的因果推理能力。实验发现现有模型在该基准上表现不佳，最佳模型准确率仅57.6%，表明当前LLM在可靠因果推理方面存在严重不足。


<details>
  <summary>Details</summary>
Motivation: 现有因果推理基准存在严重局限性，如依赖合成数据和领域覆盖狭窄。需要构建更真实、更全面的基准来评估LLM的因果推理能力。

Method: 从顶级经济金融期刊中提取因果识别关系构建基准，涵盖工具变量、双重差分、断点回归等严谨方法，包含40,379个评估项，覆盖健康、环境、技术、法律、文化等领域的五种任务类型。

Result: 在八个最先进LLM上的实验结果显示，最佳模型准确率仅为57.6%，模型规模与性能无正相关，即使是先进推理模型也难以识别基本因果关系。

Conclusion: 当前LLM能力与高风险应用中可靠因果推理需求之间存在关键差距，需要进一步改进模型在真实世界因果推理方面的表现。
...
TL;DR: LAD-RAG是一个新颖的布局感知动态RAG框架，通过构建符号文档图来捕捉视觉丰富文档的结构和跨页面依赖关系，在推理时动态检索必要证据，显著提升了多页面推理任务的检索效果和问答准确性。


<details>
  <summary>Details</summary>
Motivation: 传统RAG方法在视觉丰富文档问答中存在局限性：内容编码时丢失了结构信息和跨页面依赖关系，推理时固定检索页面数量，导致证据检索不完整和答案质量下降。

Method: 在文档摄入阶段构建符号文档图来捕捉布局结构和跨页面依赖，结合标准神经嵌入；在推理阶段使用LLM代理动态与神经和符号索引交互，自适应检索必要证据。

Result: 在多个基准测试中，LAD-RAG显著提升了检索效果，平均完美召回率超过90%，在相同噪声水平下比基线检索器召回率提升高达20%，问答准确性更高且延迟最小。

Conclusion: LAD-RAG通过结合符号和神经表示以及动态检索策略，有效解决了视觉丰富文档中多页面推理的挑战，为复杂文档问答提供了更全面的解决方案。
...
TL;DR: 该研究系统分析了五个流行的事实性基准测试和八个不同年份发布的LLM，发现由于基准测试的静态性与现实世界知识的动态演化不匹配，导致对LLM事实性评估不可靠。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型的快速发展和现实世界的动态变化已经超过了静态评估基准的更新速度，引发了对其评估LLM事实性可靠性的担忧。

Method: 使用最新的知识检索流程和三个定制化指标来量化基准测试的老化程度及其对LLM事实性评估的影响，分析了五个流行基准测试和八个不同年份的LLM。

Result: 实验结果表明，广泛使用的事实性基准测试中有相当一部分样本已经过时，导致对LLM事实性的评估不可靠。

Conclusion: 该研究为评估LLM事实性基准测试的可靠性提供了测试基础，并希望激发更多关于基准测试老化问题的研究。
...
<details>
  <summary>Details</summary>
Motivation: 现有自动化红队测试方法缺乏在推理时有效适应模型特定漏洞的机制，需要更高效识别模型失败模式的方法。

Method: 通过强化学习训练一组参数高效的LoRA专家，每个专家专攻特定攻击风格；在推理时使用多臂老虎机策略根据目标模型响应安全度动态选择攻击专家。

Result: 在充分探索条件下（ASR@10）在AdvBench上达到最先进结果，同时生成更易读的提示（更低困惑度）；老虎机策略可作为诊断工具揭示模型特定漏洞。

Conclusion: Red-Bandit框架有效解决了现有红队测试方法的局限性，提供了一种自适应、高效识别模型漏洞的方法，并可作为模型安全诊断工具。
...
TL;DR: HERO是一个结合验证器二进制反馈和奖励模型连续反馈的强化学习框架，通过分层归一化和方差感知加权来提升大语言模型的推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有的验证器只能提供0-1的二进制反馈，无法识别部分正确或替代答案，限制了学习效果。奖励模型虽然能提供更丰富的连续反馈，但缺乏验证器的可靠性。

Method: 提出HERO框架，使用分层归一化将奖励模型分数限制在验证器定义的组内，保持正确性的同时细化质量区分；采用方差感知加权来强调密集信号重要的困难提示。
...
TL;DR: 提出了一种新的无参考评估方法，将法律回答分解为'法律数据点'(LDPs)，该方法在评估法律领域LLM输出时优于现有基线，并与人类专家评估更相关。
...
Method: 将冗长回答分解为自包含的信息单元'法律数据点'(LDPs)，开发反映律师评估法律答案方式的参考无关评估方法。

Result: 在专有数据集和开源数据集(LegalBench)上均优于多种基线方法，与人类专家评估更相关，并提高了标注者间一致性。

Conclusion: 该方法填补了法律领域LLM评估的空白，开源了部分LegalBench的法律数据点，促进研究社区在该重要领域的进一步发展。
...
TL;DR: PA-Tool是一种无需训练的方法，通过调整工具模式与预训练知识对齐，解决小语言模型在工具使用中的模式不匹配问题，显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 小语言模型在工具使用任务中表现不佳，特别是在选择合适工具和识别正确参数时。主要失败模式是模式不匹配：模型会产生看似合理但实际不存在的工具名称，这些名称反映了预训练期间内化的命名约定，但在提供的工具模式中不存在。

Method: 提出PA-Tool（预训练对齐工具模式生成），一种无需训练的方法，利用峰值度（污染检测中的信号，表示预训练熟悉度）自动重命名工具组件。通过生成多个候选并选择在样本间输出浓度最高的候选，识别预训练对齐的命名模式。

Result: 在MetaTool和RoTBench上的实验显示性能提升高达17个百分点，模式不匹配错误减少80%。PA-Tool使小模型能够接近最先进性能，同时保持计算效率，无需重新训练即可适应新工具。

Conclusion: 工作表明，通过调整模式以适应模型而非模型适应模式，模式级干预可以释放资源高效模型的工具使用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于静态标准的训练方法容易受到奖励攻击，且无法捕捉训练过程中出现的需求变化

Method: 通过比较当前策略和参考策略的响应来动态生成评估标准，实现持续的错误识别和缓解

Result: 在AlpacaEval、GPQA、ArenaHard等多个基准上比静态标准方法提升8%

Conclusion: 在线动态标准生成能有效提升LLM训练效果，识别出透明度、实用性、组织性和推理能力等关键评估维度
...
TL;DR: 论文研究了LLMs的内在自我修正能力，特别是在道德自我修正方面，揭示了多轮互动导致性能收敛的机制。


<details>
  <summary>Details</summary>
Motivation: LLMs能够根据指令改进回答，但内在自我修正（仅提供抽象目标而无具体问题细节）的有效机制尚不明确，特别是在道德修正方面。

Method: 通过实验分析道德自我修正的多轮互动过程，研究性能收敛的机制。

Result: 发现持续注入的自我修正指令会激活道德概念，降低模型不确定性，随着激活的道德概念在多轮中稳定，性能趋于收敛。

Conclusion: 道德自我修正展现出性能收敛的理想特性，具有强大的应用潜力。
...
TL;DR: M-Thinker通过GRPO算法训练，结合语言一致性奖励和跨语言思维对齐奖励，解决了大型推理模型在非英语语言处理中的语言一致性和推理性能问题。


<details>
  <summary>Details</summary>
Motivation: 当前大型推理模型在处理非英语语言时存在两个关键限制：输入输出语言一致性差，以及推理路径错误导致答案准确性低于英语。这些问题影响了非英语用户的体验并阻碍了模型的全球部署。

Method: 提出M-Thinker模型，采用GRPO算法训练，包含语言一致性奖励和跨语言思维对齐奖励。语言一致性奖励确保输入、思维和答案的语言一致性；跨语言思维对齐奖励通过比较模型在非英语和英语上的推理路径，将推理能力从英语转移到非英语语言。

Result: M-Thinker-1.5B/7B模型在两个多语言基准测试（MMATH和PolyMath）上实现了近乎100%的语言一致性和优越性能，并在领域外语言上表现出优秀的泛化能力。

Conclusion: M-Thinker通过创新的训练方法有效解决了大型推理模型在非英语语言处理中的关键问题，显著提升了多语言推理性能。
...
TL;DR: CORGI是一个专为真实商业场景设计的新文本到SQL基准测试，包含受DoorDash、Airbnb和Lululemon启成的合成数据库，涵盖描述性、解释性、预测性和推荐性四类复杂商业查询。


<details>
  <summary>Details</summary>
Motivation: 现有文本到SQL基准测试主要关注过去记录的事实检索，而真实商业决策需要因果推理、时间预测和战略推荐等多层次智能能力。

Method: 构建包含企业合成数据库的CORGI基准，设计四类复杂度递增的商业查询类型，评估LLM在因果推理、时间预测和战略推荐等任务上的表现。

Result: LLM在高层级问题上的性能下降，难以做出准确预测和提供可执行计划。CORGI基准比BIRD基准难度高出约21%。

Conclusion: 当前流行LLM与真实商业智能需求之间存在显著差距，CORGI基准为评估和改进LLM在复杂商业场景下的能力提供了重要工具。
...
TL;DR: 提出了VeriCode分类法和Vibe Checker测试平台，用于评估LLM的代码指令遵循能力，发现指令遵循是除了功能正确性之外影响人类偏好的关键因素。
...
Result: 评估31个领先LLM发现，即使最强模型也难以遵循多个指令并出现功能回归。指令遵循能力是真实编程任务中区分模型表现的主要因素。

Conclusion: 功能正确性和指令遵循的综合评分与人类偏好相关性最高，为开发更符合用户编码偏好的模型提供了具体路径。
...
TL;DR: 提出了一种结合短期记忆（滑动窗口KV缓存）和长期记忆（固定大小压缩记忆）的混合记忆框架，通过人工海马网络(AHN)压缩历史信息，在保持性能的同时显著降低计算和内存需求。


<details>
  <summary>Details</summary>
Motivation: 解决长序列建模中RNN类模型的压缩记忆效率与Transformer类模型的无损记忆保真度之间的权衡问题，受认知科学中的多存储模型启发。

Method: 维护Transformer的KV缓存作为无损短期记忆，同时使用人工海马网络(AHN)将窗口外信息递归压缩为固定大小的紧凑长期记忆，使用Mamba2、DeltaNet和Gated DeltaNet等现代RNN架构实现AHN。

Result: 在LV-Eval和InfiniteBench长上下文基准测试中，AHN增强模型持续优于滑动窗口基线，性能与全注意力模型相当甚至更好，同时显著降低计算和内存需求。例如，Qwen2.5-3B-Instruct的推理FLOPs减少40.5%，内存缓存减少74.0%，LV-Eval平均得分从4.41提升到5.88。

Conclusion: AHN框架有效平衡了记忆效率和保真度，为长序列建模提供了一种高效且性能优越的解决方案。
